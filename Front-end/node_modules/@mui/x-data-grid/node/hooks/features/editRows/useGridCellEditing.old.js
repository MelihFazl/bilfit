...
const useCellEditing = (apiRef, props) => {
  ...
  const setCellMode = React.useCallback((id, field, mode) => {
    ...
    apiRef.current.publishEvent(_gridEvents.GridEvents.cellModeChange, apiRef.current.getCellParams(id, field));
  }, [apiRef, logger]);
    ...
    if (!hasError && typeof column.preProcessEditCellProps === 'function') {
      ...
      if (isPromise(result)) {
        return result.then(newEditCellProps => {
          ...
          apiRef.current.publishEvent(_gridEvents.GridEvents.cellEditCommit, commitParams, event);
          return true;
        });
      }
  ...
  const handleCellKeyDown = React.useCallback(async (params, event) => {
    ...
    if (!isEditMode && (0, _keyboardUtils.isCellEnterEditModeKeys)(event.key) && !isModifierKeyPressed && !(event.key === ' ' && event.shiftKey)) {
      apiRef.current.publishEvent(_gridEvents.GridEvents.cellEditStart, params, event);
    }
    ...
    if (!isEditMode && (0, _keyboardUtils.isDeleteKeys)(event.key)) {
      ...
      apiRef.current.publishEvent(_gridEvents.GridEvents.cellEditStop, params, event);
    }
  ...
  (0, _useGridApiEventHandler.useGridApiEventHandler)(apiRef, _gridEvents.GridEvents.cellEditStart, buildCallback(handleCellEditStart));
  (0, _useGridApiEventHandler.useGridApiEventHandler)(apiRef, _gridEvents.GridEvents.cellEditStop, buildCallback(handleCellEditStop));
  (0, _useGridApiEventHandler.useGridApiEventHandler)(apiRef, _gridEvents.GridEvents.cellEditCommit, buildCallback(handleCellEditCommit));
  ...
  (0, _useGridApiEventHandler.useGridApiOptionHandler)(apiRef, _gridEvents.GridEvents.cellEditCommit, props.onCellEditCommit);
